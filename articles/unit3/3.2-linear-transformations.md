...
_A 2×2 matrix with adjustable entries. Shows the unit grid and how it transforms in real-time as user changes the matrix._
...
- (B) $\begin{bmatrix} 5 & 0 \\ -2 & 3 \end{bmatrix}$
...
- First column: $\begin{bmatrix} 5 \\ -2 \end{bmatrix}$
- Second column: $\begin{bmatrix} 0 \\ 3 \end{bmatrix}$

So $A = \begin{bmatrix} 5 & 0 \\ -2 & 3 \end{bmatrix}$.
...
**[PLACEHOLDER: Interactive widget]**
_3D cube being transformed by a 3×3 matrix. User can adjust matrix entries._
...
We've seen that matrices are transformations. But what if we want to *undo* a transformation? What if we want to go backwards?

In the next section, we'll explore **matrix inverses** — when they exist, how to find them, and what it means geometrically to "reverse" a linear transformation.


---

[← Previous: Matrix-Vector Multiplication](3.1-matrix-vector-multiplication.md) | [Back to Home](/) | [Next: Matrix of a Transformation →](3.3-matrix-of-transformation.md)
